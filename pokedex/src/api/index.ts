...
async function fetchDataWithCaching<T>(
  ...
  try {
    ...
    if (Array.isArray(data.errors) && data.errors.length) {
      showToast({ title: "API Error", message: data.errors[0].message, style: Toast.Style.Failure });
    ...
    showToast({ title: "Network Error", message: e.message, style: Toast.Style.Failure });
